...
{ 
  ...
  led.op_ramp(time_6ms, UP, 35); //increase pwm 35 times with 6ms interval
  led.op_ramp(time_1ms5, DOWN, 35); //decrease pwm 35 times with 1.5ms interval
  led.op_wait(time_15ms); //wait 15ms
  led.op_mux_map_next(); 
  led.op_ramp(time_3ms, DOWN, 15);
  led.op_ramp(time_0ms5, UP, 100);
  led.op_ramp(time_0ms5, DOWN, 120);
  led.op_ramp(time_1ms, UP, 35);
  led.op_wait(time_15ms);
  led.op_mux_map_next();
  led.op_ramp(time_2ms, UP, 39);
  led.op_ramp(time_1ms, DOWN, 39);
  led.op_wait(time_368ms);

...
const char mainPage[] PROGMEM = { //timer reading humidity value every 2s
"<body>\r\n"
"<textarea rows='1' cols='50' id='data'></textarea><br>\r\n"
"<button type='button' onclick=\"timer2sec = setInterval(function(){sndCmd('humi', 'data')}, 2000);\">Get data</button>\r\n"
...
void setup() {
  ...
  humidity.init();
  delay(1);
  humidity.readCalibration();
  Serial1.begin(115200);
  while(!Serial1);
  if(enterAtMode()){ //check if device is set to 115200baud and if so, set it to 38400baud and WMODE & NETP settings to correct values
    Serial1.print("AT+WMODE=APSTA\r");
    Serial1.find("+ok");
    Serial1.print("AT+UART=38400,8,1,None,NFC\r");
    Serial1.find("+ok");
    Serial1.print("AT+NETP=TCP,Server,33170,10.10.100.100\r");
    Serial1.find("+ok");
...
void readWifi(void){
  ...
  while(Serial1.available()){
    ...
    else if (htmlHeader.indexOf("@5FA802")>-1)
        aistinHumiData();
    else if (htmlHeader.indexOf("@5FB010")>-1)
        aistinHumiCalibration();
...
void customCommand(void){
  ...
  else if(cmd.indexOf("humi")==0){
    int humi;
    humi = humidity.readHumidity();
...
void aistinSensorResponse(void){
  ...
  Serial1.println(">D~Aistin@8038:805F");
}
void aistinHumiData(void){
  uint16_t humi = humidity.readHumidityRaw();
  Serial1.print(">D~Aistin@5FA802:");
  Serial1.print((humi>>4)%16,HEX);
  Serial1.print(humi%16,HEX);
  Serial1.print((humi>>12)%16,HEX);
  Serial1.println((humi>>8)%16,HEX);
}

void aistinHumiCalibration(void){
  uint8_t humiCalib[16];
  humidity.readReg(0xB0,humiCalib,16);
  Serial1.print(">D~Aistin@5FB010:");
  for(int i=0;i<16;i++){
    Serial1.print((humiCalib[i]>>4)%16,HEX);
    Serial1.print(humiCalib[i]%16,HEX);
  }
  Serial1.println();  
}
